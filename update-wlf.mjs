...
async function scrape() {
  ...
  // Merge con esistente usando il NUMERO del concorso come chiave primaria
  let existing = [];
  if (fs.existsSync(DATA_FILE)) {
    existing = JSON.parse(fs.readFileSync(DATA_FILE, 'utf-8'));
  }

  // Crea un Set con i numeri di concorso già presenti (ignorando eventuali undefined)
  const existingKeys = new Set(existing.map(d => d.numero).filter(n => n !== undefined));

  let added = 0;
  for (const d of finalDraws) {
    if (d.numero && !existingKeys.has(d.numero)) {
      existing.push(d);
      existingKeys.add(d.numero);
      added++;
    } else if (!d.numero) {
      // Se il record non ha un numero di concorso (caso improbabile), controlla per datetime
      const exists = existing.some(e => e.datetime === d.datetime);
      if (!exists) {
        existing.push(d);
        added++;
      }
  ...
  console.log(`Aggiunte ${added} nuove estrazioni. Totale: ${existing.length} estrazioni.`);
}
